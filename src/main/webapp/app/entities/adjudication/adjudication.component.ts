...
export class AdjudicationComponent implements OnInit, OnDestroy {
  ...
  loadAll() {
    this.adjudicationService
      ...
      .subscribe(
        (res: HttpResponse<IAdjudication[]>) => this.paginateAdjudications(res.body, res.headers),
  ...
  transition() {
    this.router.navigate(['/adjudication'], {
      queryParams: {
        page: this.page,
        size: this.itemsPerPage,
  ...
  trackId(index: number, item: IAdjudication) {
    return item.id;
  }

  registerChangeInAdjudications() {
    this.eventSubscriber = this.eventManager.subscribe('adjudicationListModification', response => this.loadAll());
  }
  ...
  protected paginateAdjudications(data: IAdjudication[], headers: HttpHeaders) {
    this.links = this.parseLinks.parse(headers.get('link'));
    this.totalItems = parseInt(headers.get('X-Total-Count'), 10);
    this.adjudications = data;
  }
